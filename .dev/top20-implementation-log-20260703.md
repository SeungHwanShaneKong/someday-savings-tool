...
## SEC-AUDIT 최종 오라클
tsc 0 · vitest **1471/1471**(+27 보안테스트) · build:ssg 23 · 시각 16 green · 독립검증 GO(배치+#5+#6 서브검증). 커밋+푸시+GitHub Pages 배포 진행(사용자 명시 승인). → 배포 성공(run 28665390731, build 48s+deploy 23s, apex 200·manifest 200 라이브).

---

# BTN-AUDIT + SEC-AUDIT R2 (2026-07-03, 2차 배포)

## 전 버튼 기능 감사 (워크플로 wkziuru1r: 12표면 finder + 적대검증)
- 96 버튼 감사 → "고장 9건" 중 **8건 FALSE-POSITIVE**(라이브 검증으로 규명): 트레일링 슬래시 라우트 7건(/faq/·/terms/·/editorial/ 등) + /guide/wedding-cost-data/ 아티클 링크 1건. finder들이 React Router v6 **트레일링 슬래시 정규화**를 오해(v6는 `/faq/`→`path="/faq"` 매칭). **검증 없이 App.tsx 라우트를 고쳤다면 라이브 회귀 유발**. 프리뷰 라이브 렌더(h1 정상·404 아님) + 기존 e2e(public-pages) 통과가 반증.
- **실제 개선 2건 근본수정(TDD)**:
  - **useCountUp rAF 스로틀 강건성**: 숨겨진/비활성 탭(document.hidden)에서 rAF 콜백 미발화 → 시각 카운트업이 stale 고정되어 aria-live 최종값과 발산(프리뷰 hidden 탭에서 시뮬레이터 총액 7,247만원 고정으로 관측). 근본수정 = hidden 탭이면 즉시 최종값 스냅 + durationMs 폴백 타이머(중도 hidden 전환도 최종 일관성 보장). RED(rAF no-op mock)→GREEN.
  - **BadgeChip a11y**: 표시용 배지가 `<button disabled>`로 렌더돼 SR이 획득 배지를 "비활성"으로 오안내 + 포커스 불가. 근본수정 = onClick 있으면 button / 없으면 `role="img"` 비인터랙티브(disabled 제거). BadgeGrid/BadgeChip 테스트 role 갱신(button→img).

## SEC-AUDIT R2 확장 렌즈(워크플로 wnov8sosr: 동시성·경계·에러·성능·a11y·빌드 6렌즈)
- 22 후보 → **확정 2**(false_positive 18·already_mitigated 2). **치명 0**(확장 렌즈로도).
- **#1 위저드 배치 부분실패 → PM 검증으로 FALSE-POSITIVE**: 검증관이 "updateItem re-throw"로 오판했으나 실제 updateItem(useMultipleBudgets:581-618)은 catch에서 롤백+토스트로 내부 처리·**no re-throw** → updateAmount 절대 reject 안 함 → 위저드 루프 조기종료 없이 전 prefill 적용(BudgetFlow:329 주석 명시). PM이 workflow verifier를 재검증해 기각.
- **#2 useChatBudgetSummary 무음 실패 → 근본수정**: 조회 실패가 setSummary(null)만 하고 로그 0 → 디버깅 불가. 근본수정 = '예산 없음'(신규 유저 정상)과 '조회 실패'(네트워크/RLS) 구분해 실패만 console.debug(useAIChat 패턴). degrade-safe 동작 불변. 신규 테스트 2(실패 로깅·정상 무로깅).

## 2차 배포 최종 오라클
tsc 0 · vitest **1475/1475**(+4: useCountUp hidden·BadgeChip E6·useChatBudgetSummary 2) · build:ssg 23 · 시각 16. 커밋+푸시+배포(사용자 명시 승인).
**교훈**: 정적 버튼 감사는 프레임워크 동작(RR 트레일링슬래시)을 오해해 대량 false-positive 생성 가능 → **라우트/라이브 동작은 반드시 프리뷰·e2e로 실증 후 수정**(검증 없는 일괄수정은 라이브 회귀 위험). `.dev/lessons-learned.md` 적재.
