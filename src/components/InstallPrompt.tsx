...
// [CL-PWA-A2HS-20260706-202720] 코어 시스템 통일 — useInstallResolution·계측·InstallGuideDialog 연결(핸들러 단일 경로).
// 마운트는 PM(App.tsx)이 담당 — 여기서는 export 만.
import { useCallback, useState } from 'react';
import { Download, Share, SquarePlus, X } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { useIsMobile } from '@/hooks/use-mobile';
import { isInstallPromptSuppressed, rememberInstallPromptDismissed } from '@/hooks/usePWAInstall';
...
export function InstallPrompt() {
  ...
  const isInstallable = resolution.canOneTap;
  const isIOS = resolution.platform === 'ios';
  ...
  const handleInstall = useCallback(async () => {
    trackFunnel('pwa_install_cta_click', { placement: 'banner', platform: resolution.platform });
    const outcome = await promptInstall();
    if (outcome === 'accepted') {
      trackFunnel('pwa_install_accepted', { placement: 'banner' });
      // 억제 기록 발화 — appinstalled→standalone 전환이 지연돼도 FAB 조율이 일관되게(배너→FAB) 유지되도록.
      rememberInstallPromptDismissed();
      setDismissed(true);
    } else if (outcome === 'dismissed') {
      // 네이티브 프롬프트에서 거절해도 같은 30일 억제 적용(반복 노출 방지)
      trackFunnel('pwa_install_dismissed', { placement: 'banner' });
      rememberInstallPromptDismissed();
      setDismissed(true);
    } else {
      // 이벤트 이미 소진 등 → 수동 안내로 폴백(배너는 유지)
      setGuideOpen(true);
    }
  }, [promptInstall, resolution.platform]);
  ...
  // 미지원 브라우저(이벤트 미발생·iOS 아님)는 렌더하지 않음 — 이 경우 전역 InstallFab 이 접근성 담당
