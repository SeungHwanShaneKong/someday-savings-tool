...
// [CL-AUDIT-POKE-D1-20260711] 모바일 분기는 useIsMobile 목킹이 아니라 **실물 window.innerWidth** 로 제어한다.
//   근거: 컴포넌트가 useIsMobile(비동기 첫-렌더 false) 대신 effect 시점 window.innerWidth 를 직접 읽도록
//   근본수정됨 → 목킹으로 첫-렌더 레이스를 우회하던 false-green 을 제거(실물 경로 회귀 가드).
...
beforeEach(() => {
  localStorage.clear();
  setViewport(1024); // 기본 데스크톱(모바일 배타 미발동)
...
describe('PokeNudgeCard — InstallPrompt 시간 배타(모바일)', () => {
  it('NC.11 모바일 + PWA 배너 미억제 → 미렌더 + 일일 키 미기록(기회 보존)', () => {
    // [CL-AUDIT-POKE-D1-20260711] 실물 모바일 뷰포트 — 근본수정 전(useIsMobile 첫-렌더 false)엔 카드가
    //   렌더돼 RED. effect 시점 window.innerWidth 직독으로 수정되면 배타가 정확히 발동해 GREEN.
    setViewport(375);
    renderWithProviders(<PokeNudgeCard {...baseProps} />);
    expect(queryCard()).toBeNull();
    expect(localStorage.getItem(SHOWN_KEY)).toBeNull();
