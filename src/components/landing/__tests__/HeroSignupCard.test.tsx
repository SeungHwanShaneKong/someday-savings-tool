...
// [CL-HERO-SUMMARY-20260718] 카드 미니 요약이 실데이터 단일소스와 정합함을 동적 검증(하드코딩 표류 가드)
...
describe('HeroSignupCard — 렌더 계약', () => {
  it('H1: Google CTA·예산 스냅샷(총액/예시 배지)·신뢰 칩·개인정보 링크가 렌더된다', () => {
    ...
    // 데이터 파생 미니 엑셀 요약(하드코딩 아님) — 총액·카테고리는 단일소스에서 계산
    const cats = buildSampleBudget();
    const topCat = [...cats].sort((a, b) => b.subtotal - a.subtotal)[0];
    expect(screen.getByText(formatKoreanWon(sampleBudgetTotal(cats)))).toBeInTheDocument();
    expect(screen.getByText('예시')).toBeInTheDocument();
    expect(screen.getByText(topCat.name)).toBeInTheDocument();
    // 읽기전용 — 편집 컨트롤(input) 0
...
describe('HeroSignupCard — Google 직접 로그인', () => {
  ...
  it('H5: 진행 중에는 "연결 중..." + disabled + aria-busy, 완료 시 원복(더블서밋 게이트)', async () => {
    ...
    await screen.findByRole('button', { name: 'Google로 5초 만에 시작' });
